locals {
  ...
  access_entry_map = merge(
    {
      (one(module.github_actions_iam_role[*].outputs.arn)) = {
  ...
  # karpenter_role_arn = one(aws_iam_rolaccess_entry_mape.karpenter[*].arn)
  ...
  linux_worker_role_arns = [] # local.enabled && local.karpenter_iam_role_enabled ? [local.karpenter_role_arn] : []
...
#   policy_description = "Allow full access to the EKS cluster"
#   role_description   = "IAM role with permissions to perform actions on EKS resources"
...
#   policy_documents = [
#     data.aws_iam_policy_document.eks_full_access.json
#   ]
# }
